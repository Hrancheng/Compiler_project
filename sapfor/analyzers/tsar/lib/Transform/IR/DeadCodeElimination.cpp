...
bool NoMetadataDSEPass::runOnFunction(Function &F) {
  ...
  //         bool allReturnsConstant = true;
  //         Constant *constantValue = nullptr;
            //  if (!(!F->isDeclaration() && !F->empty())) {
            //    callInst->replaceAllUsesWith(constantValue);
            //  }
  //         for (auto &block : *calledFunc) {
  //           for (auto &inst : block) {
  //             if (auto *retInst = dyn_cast<ReturnInst>(&inst)) {
  //               if (auto *returnValue = dyn_cast<Constant>(retInst->getReturnValue())) {
  //                 if (!constantValue) {
  //                   constantValue = returnValue;
  //                 } else if (constantValue != returnValue) {
  ...
  //             }
  //           }
  //           if (!allReturnsConstant) break;
  //         }
  //         if (allReturnsConstant && constantValue) {
  //           callInst->replaceAllUsesWith(constantValue);
